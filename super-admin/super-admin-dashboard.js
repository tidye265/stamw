...
if (!token) window.location.href = 'super-admin.html';
...
function selectMenu(section) {
    ...
    document.querySelectorAll('.sidebar-item').forEach(item => item.classList.toggle('active', item.dataset.section === section));
    loadData();
}
function logout() {
...
async function fetchData(section) {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/collect-data-api?section=${section}`, { headers: { 'Authorization': 'Bearer ' + token } });
...
async function collectCommissions() {
    try {
        ...
        const data = await res.json();
        if (data.success) {
            showToast('Commissions collected: MK ' + Number(data.total_commissions).toFixed(2));
...
// Attach collect button (if exists)
document.addEventListener('DOMContentLoaded', () => {
    const collectBtn = document.getElementById('collectCommissionsBtn');
    if (collectBtn) {
        collectBtn.addEventListener('click', collectCommissions);
    }
});
...
function hideSkeleton() {
    ...
    const target = document.getElementById(currentSection + 'Section');
    if (target) target.classList.add('active');
}
...
function renderAdmins(data) {
    ...
    body.innerHTML = cachedAdmins.map(a => {
        ...
        <tr>
            ...
            <td>${a.total_films} (${a.films ? a.films.map(f => f.title).join(', ') : 'No films'})</td>
            <td>${a.admins_verify ? '✅' : '❌'}</td>
            <td>
...
function filterLogins() {
    ...
    const filtered = cachedSessions.filter(s => s.token.toLowerCase().includes(query) || (s.ip_address || '').toLowerCase().includes(query) || (s.device_info || '').toLowerCase().includes(query));
...
function filterUsers() {
    ...
    const filtered = cachedUsers.filter(u => u.full_name.toLowerCase().includes(query) || u.phone.toLowerCase().includes(query) || u.user_id.toLowerCase().includes(query));
...
function filterAdmins() {
    ...
    const filtered = cachedAdmins.filter(a => a.full_name.toLowerCase().includes(query) || a.email.toLowerCase().includes(query) || a.admin_id.toLowerCase().includes(query));
...
function closeModal() { document.getElementById('balanceModal').classList.remove('open'); }
async function submitBalanceAdjust() {
    ...
    try {
        if (balanceTarget.type === 'user') await postAction('update_user_balance', { user_id: balanceTarget.id, amount, operation });
        else if (balanceTarget.type === 'admin') await postAction('update_admin_balance', { admin_id: balanceTarget.id, amount, operation });
...
// ===== ACTIONS =====
async function deleteUser(userId) { if (!confirm('Delete this user?')) return; try { await postAction('delete_user', { user_id: userId }); showToast('User deleted'); loadData(); } catch (e) { showToast(e.message, 'error'); } }
async function deactivateUser(userId) { if (!confirm('Deactivate this user?')) return; try { await postAction('deactivate_user', { user_id: userId }); showToast('User deactivated'); loadData(); } catch (e) { showToast(e.message, 'error'); } }
async function activateUser(userId) { if (!confirm('Activate this user?')) return; try { await postAction('activate_user', { user_id: userId }); showToast('Welcome back 🎉'); loadData(); } catch (e) { showToast(e.message, 'error'); } }
async function deleteAdmin(adminId) { if (!confirm('Delete this admin?')) return; try { await postAction('delete_admin', { admin_id: adminId }); showToast('Admin deleted'); loadData(); } catch (e) { showToast(e.message, 'error'); } }
async function deactivateAdmin(adminId) { if (!confirm('Deactivate this admin?')) return; try { await postAction('deactivate_admin', { admin_id: adminId }); showToast('Admin deactivated'); loadData(); } catch (e) { showToast(e.message, 'error'); } }
async function activateAdmin(adminId) { if (!confirm('Activate this admin?')) return; try { await postAction('activate_admin', { admin_id: adminId }); showToast('Welcome back 🎉'); loadData(); } catch (e) { showToast(e.message, 'error'); } }
...
async function loadData() {
    ...
    try {
        ...
        if (err.message.includes('Invalid') || err.message.includes('expired')) setTimeout(() => window.location.href = 'super-admin.html', 2000);
    }
}
