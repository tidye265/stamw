...
(function() {
  ...
  const sidebarFilmsCountBadge = document.getElementById('sidebarFilmsCount'); // Badge ya sidebar
  ...
  async function loadStats() {
    ...
    try {
      ...
      // 5. Net Followers (placeholder)
      netFollowersEl.textContent = '0';

      // 6. Films
      const films = data.films || [];
      filmsCountBadge.textContent = films.length;
      sidebarFilmsCountBadge.textContent = films.length; // Update sidebar badge too
      loadFilmsTable(films);
      loadAllFilmsList(films);

    } catch (error) {
      console.error('Error loading stats:', error);
      // Hide skeleton even on error, show alert
      hideSkeleton();
