...
(function() {
  ...
  async function loadStats() {
    ...
    try {
      ...
      // 2. Total Earnings (Queued)
      if (totalEarningsEl) {
        const queuedEarnings = Number(data.admin.queued_earnings || 0);
        totalEarningsEl.textContent = formatCurrency(queuedEarnings);
      ...
      // 4. Net Followers (followers_counter)
      if (netFollowersEl) {
        const followers = Number(data.admin.followers_counter || 0);
        netFollowersEl.textContent = followers.toLocaleString();
        netFollowersEl.style.color = 'var(--green-success)';
      }

      // 5. Films
      const films = data.films || [];
      allFilms = films; // Store for search
  ...
  function renderBarChart(films) {
    ...
    films.forEach(film => {
      ...
      const title = safeString(film.title, 'Untitled');
  ...
  // Load films table with earnings (and support search)
  function loadFilmsTable(films) {
    ...
    films.forEach(film => {
      const title = safeString(film.title, 'Untitled');
      const episode = film.episode_number ? 'EP ' + film.episode_number : 'Movie';
      const price = formatCurrency(film.price);
      const earnings = formatCurrency(film.total_earnings || 0);
  ...
  window.filterFilms = function() {
    ...
    const filtered = allFilms.filter(film => 
      (safeString(film.title, '').toLowerCase().includes(query)) || 
      (film.episode_number ? 'ep ' + film.episode_number : 'movie').toLowerCase().includes(query)
    );
