...
(function() {
  ...
  async function loadStats() {
    ...
    try {
      // Call admin-stats-api (no more transactions.film_id error!)
      ...
      queuedEarningsEl.style.color = 'var(--text-muted)'; // Grey

      // 4. Revenue Fee
      revenueFeeEl.textContent = '30%';

      // 5. Transactions
      transactionsEl.textContent = data.films.length;

      // 6. Load Films Table
      loadFilmsTable(data.films);
  ...
  withdrawBtn.addEventListener('click', function() {
    ...
    if (isNaN(numAmount) || numAmount <= 0) { alert('Invalid amount.'); return; }
    alert('Withdrawal request of MWK ' + numAmount.toLocaleString() + ' submitted! It will be processed within 24 hours.');
  });
