...
(function() {
  ...
  const errorBox = document.getElementById('errorBox'); // ✅ Already exists
  ...
  async function loadStats() {
    ...
    try {
      ...
      hideSkeleton();
      errorBox.style.display = 'none'; // Hide error if present
      ...
      hideSkeleton();
      const errMsg = error.message || 'Failed to load stats';

      // Check if error is about deactivation/approval
      if (error.message.includes('deactivated') || error.message.includes('not approved') || error.message.includes('Invalid session')) {
        // Show "WAIT APPROVED" template and stop polling
        errorBox.style.display = 'block';
        errorBox.innerHTML = `
          <div class="error-msg" style="background:#2a1a1a; color:#ff4d4d; padding:20px; border-radius:8px; text-align:center;">
            <h3 style="margin-bottom:10px;">⏳ Wait for Approval</h3>
            <p>Your account is currently not approved or has been deactivated. Please contact support.</p>
          </div>
        `;
      ...
      showToast('Failed to load stats: ' + errMsg, 'error');
    }
  }
