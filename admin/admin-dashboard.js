...
(function() {
  ...
  const approvalTemplate = document.getElementById('approvalTemplate');
  ...
  function showPendingState() {
    // Hide everything except approval template
    if (adminHeader) adminHeader.style.display = 'none';
    if (sidebarOverlay) sidebarOverlay.style.display = 'none';
    if (sidebar) sidebar.style.display = 'none';
    if (mainContent) mainContent.style.display = 'none';
    if (poweredByFooter) poweredByFooter.style.display = 'none';
    if (approvalTemplate) approvalTemplate.style.display = 'flex';
  }
  ...
  function showNormalState() {
    ...
    if (poweredByFooter) poweredByFooter.style.display = 'block';
    if (approvalTemplate) approvalTemplate.style.display = 'none';
  }
  ...
  async function loadStats() {
    ...
    try {
      ...
      // Check for pending/deactivated
      if (res.status === 403) {
        if (data.deactivated || data.not_approved) {
          showPendingState();
          return;
        }
  ...
  function renderBarChart(films) {
    ...
    // Use film.earnings, fallback to total_earnings if needed
    const maxEarnings = Math.max(...films.map(f => Number(f.earnings || f.total_earnings || 0) || 0), 1);

    films.forEach(film => {
      const bar = document.createElement('div');
      bar.className = 'chart-bar';
      const earnings = Number(film.earnings || film.total_earnings || 0) || 0;
  ...
  // ===== LOAD FILMS TABLE (Uses earnings) =====
  function loadFilmsTable(films) {
    ...
    films.forEach(film => {
      ...
      row.innerHTML = `
        ...
        <td>${formatCurrency(film.earnings || 0)}</td>
        <td>
  ...
  // ===== SEARCH FILMS =====
  window.filterFilms = function() {
    const query = (filmSearchInput.value || '').toLowerCase().trim();
    const filtered = allFilms.filter(film => 
      (film.title || '').toLowerCase().includes(query) || 
      (film.episode_number ? 'episode ' + film.episode_number : 'movie').includes(query)
    );
    loadFilmsTable(filtered);
  };

  if (filmSearchInput) {
    filmSearchInput.addEventListener('input', window.filterFilms);
  }
