...
(function() {
  ...
  const filmsSearchInput = document.getElementById('filmsSearchInput');
  ...
  // Store current films for search
  let currentFilms = [];
  ...
  async function loadStats() {
    ...
    try {
      // Use v1-admin-api instead of admin-stats-api
      const res = await fetch(`${SUPABASE_URL}/functions/v1/v1-admin-api`, {
        method: 'GET',
      ...
      // 1. Available Balance (wallet_balance)
      ...
      // 2. Total Earnings (Queued) – now showing queued_earnings
      if (totalEarningsEl) {
      ...
      // 4. Net Followers (fetched from backend)
      if (netFollowersEl) {
        const followers = Number(data.admin.followers_count || 0);
        netFollowersEl.textContent = followers;
      }
  ...
  // ===== SEARCH FILMS =====
  function filterFilmsTable() {
    const query = (filmsSearchInput.value || '').toLowerCase().trim();
    const filtered = currentFilms.filter(film => 
  ...
  if (filmsSearchInput) {
    filmsSearchInput.addEventListener('input', filterFilmsTable);
  }
