...
(function() {
  ...
  // ===== BALANCE CALCULATOR (Real-time) =====
  function updateCalculator() {
    const amount = parseFloat(amountInput.value) || 0;
    const adminShare = amount * 0.70;
    const platformFee = amount * 0.30;
    calcReceive.textContent = 'MWK ' + adminShare.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    calcFee.textContent = 'MWK ' + platformFee.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    calcTotal.textContent = 'MWK ' + adminShare.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  }
  ...
  submitBtn.addEventListener('click', async function() {
    ...
    try {
      ...
      showToast({
        ...
        message: `MWK ${amount.toLocaleString()} will be sent to ${network.toUpperCase()} ${phone} within 24 hours.`
      });
      ...
      submitBtn.innerHTML = '<i class="bi bi-send me-2"></i> Submit Withdrawal Request';
  ...
  async function loadBalance() {
    try {
      ...
      if (data.success && data.admin) {
        ...
        calcTotal.textContent = 'MWK ' + balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      }
    } catch (err) {
