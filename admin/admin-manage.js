...
(function() {
  ...
  // NEW: Latest checkbox
  const markAsLatest = document.getElementById('markAsLatest');
  ...
  operationSelect.addEventListener('change', function() {
    if (this.value === 'continue') {
      ...
      loadFilms();
      // Default latest to false for continue
      if (markAsLatest) markAsLatest.checked = false;
    } else {
      filmListContainer.style.display = 'none';
      existingFilmSelect.innerHTML = '';
      existingFilmSearch.value = '';
      resetForm();
      // Default latest to true for new
      if (markAsLatest) markAsLatest.checked = true;
    }
  });
  ...
  existingFilmSelect.addEventListener('change', async function() {
    ...
    // Preserve latest status from existing film
    if (markAsLatest) markAsLatest.checked = data.latest === true;
  });
  ...
  form.addEventListener('submit', async function(e) {
    ...
    const latest = markAsLatest ? markAsLatest.checked : (operation === 'new'); // default true for new, false for continue
    ...
    const verify = confirm(
      ...
      `Translator: ${admin.full_name || 'Unknown'}\n` +
      `Mark as Latest: ${latest ? 'Yes' : 'No'}`
    );
    if (!verify) return;
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      const payload = {
        ...
        translator_name: admin.full_name || 'Unknown', // NEW: Admin name as translator
        latest: latest // NEW: Latest boolean
      };
