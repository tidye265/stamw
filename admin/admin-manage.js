// admin-manage.js (updated with improved UI and success modal)
...
(function() {
  ...
  function renderFilmOptions(films) {
    ...
    films.forEach(film => {
      ...
      // If it's part of a series (episode_number exists), show "Title – Part X"
      if (film.episode_number !== null && film.series_id) {
        option.textContent = `${film.title} – Part ${film.episode_number}`;
      } else {
  ...
  function showSuccessModal({ title, message }) {
    ...
    modalTitle.textContent = title;
    modalMessage.innerHTML = message; // Use innerHTML to allow icons
  ...
  form.addEventListener('submit', async function(e) {
    ...
    // Show success modal with nice icons
    let successMsg = `<div class="text-center mb-3"><i class="bi bi-check-circle-fill" style="font-size:4rem; color:#4caf50;"></i></div><p><strong>Successfully uploaded! 🎉</strong></p><p>${uploadedCount} video(s) uploaded.</p>`;
    if (seriesId) {
      successMsg += `<p class="small text-muted">Series ID: <code>${seriesId}</code></p>`;
    }
    showSuccessModal({
