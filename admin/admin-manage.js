// admin-manage.js (updated with series display and success modal)
...
(function() {
  ...
  async function loadFilms() {
    ...
    // Fetch id, title, episode_number, series_id to display proper label
  ...
  function renderFilmOptions(films) {
    ...
    films.forEach(film => {
      ...
      // If series (has series_id), display title + "Part X"
      let label = film.title;
      if (film.series_id && film.episode_number) {
        label += ` Part ${film.episode_number}`;
      }
  ...
  form.addEventListener('submit', async function(e) {
    ...
    const successMsg = `Successfully uploaded 🎉 ${uploadedCount} video(s) uploaded.`;
    await showSuccessModal({
      title: 'Upload Complete',
      message: successMsg + (seriesId ? `\nSeries ID: ${seriesId}` : '')
    });
