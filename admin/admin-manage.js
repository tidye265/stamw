...
(function() {
  ...
  function updatePriceLabel() {
    if (episodeRange && episodeRange.value.trim() !== '') {
      priceLabel.textContent = 'Price per Episode';
    } else {
      priceLabel.textContent = 'Full Price';
    }
  }
  episodeRange.addEventListener('input', updatePriceLabel);
  // Initial call
  updatePriceLabel();
  ...
  existingFilmSelect.addEventListener('change', async function() {
    ...
    // For genres, we'll just set to empty (multi-select can't be pre-filled easily)
    // For categories, we'll also set empty
    document.getElementById('filmYear').value = data.year || '';
    // Duration removed
    document.getElementById('filmPrice').value = data.price || '';
    // Category is multi-select, we can't easily pre-fill; leave empty
    document.getElementById('filmDescription').value = data.description || '';
    if (data.poster_url) {
      previewImg.src = data.poster_url;
      preview.style.display = 'block';
    }
    // NEW: set latest checkbox based on existing value
    if (latestCheck) {
      latestCheck.checked = data.latest === true;
    }
  });
  ...
  function resetForm() {
    ...
    previewImg.src = '';
    // NEW: reset latest to default true for new uploads
    if (latestCheck) {
      latestCheck.checked = true;
    }
  ...
  function getSelectedCategories() {
    const selected = Array.from(document.getElementById('filmCategory').selectedOptions).map(o => o.value);
    return selected.join(',');
  }
  ...
  form.addEventListener('submit', async function(e) {
    ...
    // Duration removed – default to 0 or null
    const duration = null; // or 0
    const price = parseFloat(document.getElementById('filmPrice').value);
    const category = getSelectedCategories(); // Multi-select joined
    const description = document.getElementById('filmDescription').value.trim();
    const genres = getSelectedGenres();
    const episodeRangeValue = document.getElementById('episodeRange').value.trim();
    const posterFileInput = posterFile.files[0];
    // poster URL removed, so we only have file upload

    // NEW: translator_name from admin.full_name, latest from checkbox
    const translator_name = admin.full_name || null;
    const latest = latestCheck ? latestCheck.checked : false;

    const episodeNumbers = generateEpisodeNumbers(episodeRangeValue);
    ...
    const verify = confirm(
      ...
      `Translator: ${translator_name || 'N/A'}\n` +
    ...
    // 1. Upload Poster (file only, no URL fallback)
    if (!posterFileInput) {
      alert('Please provide a poster file for new film.');
      return;
    }
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      const payload = {
        ...
        // NEW: include translator_name and latest
