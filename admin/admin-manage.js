...
(function() {
  ...
  form.addEventListener('submit', async function(e) {
    ...
    let seriesId = null; // This will hold the ID of the first episode (used as series_id for later episodes)
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      const moviePayload = {
        ...
        series_id: seriesId, // Use the first episode's ID as series_id for subsequent episodes
      ...
      // If first episode in a new series, store its ID as seriesId
      if (!seriesId && operation === 'new' && queue.length > 1) {
        seriesId = createData.movie.id;
      }
