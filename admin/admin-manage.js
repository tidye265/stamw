...
(function() {
  ...
  // NEW: Price label and latest checkbox
  ...
  existingFilmSelect.addEventListener('change', async function() {
    ...
    // Set latest checkbox based on existing value
    if (latestCheck) {
  ...
  function resetForm() {
    ...
    preview.style.display = 'none';
    previewImg.src = '';
    // Reset latest to default true for new uploads
    if (latestCheck) {
  ...
  form.addEventListener('submit', async function(e) {
    ...
    // NEW: Default duration 120 if not provided
    const duration = 120; // Default duration
    const price = parseFloat(filmPriceInput.value);
    const category = getSelectedCategories(); // Multi-category
    ...
    if (!posterFileInput) {
      alert('Please upload a poster file.');
      return;
    }
    if (queue.length === 0) {
