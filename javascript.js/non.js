(function() {
    ...
    var searchInitialized = false; // Flag to prevent double init
    ...
    // ===== FIXED DEDUPLICATION (TRIM NAMES) =====
    function getUniqueTranslators() {
        var translators = {};
        allFilms.forEach(function(m) {
            if (m.translator_name) {
                var name = m.translator_name.trim(); // Remove trailing/leading spaces
    ...
    function showContent() {
        ...
        // Initialize search AFTER content is shown (so button is visible)
        initSearch();
    ...
    async function fetchMovies() {
        try {
            ...
            // My Movies badge always 0 on non.html
    ...
    // ===== SEARCH FUNCTIONALITY (Search title OR translator) =====
    function initSearch() {
        if (searchInitialized) return; // Prevent double init
    ...
    function init() {
        ...
        // Initialize categories and card click (these work even when hidden)
        initCategories();
        initCardClick();

        // Fetch movies (will also init search & infinite scroll after content shows)
        fetchMovies();
    }
