...
(function() {
    ...
    // ===== CATEGORY MATCHING (Supports multiple categories like home.js) =====
    ...
    function getFilteredFilms() {
        var items = allFilms;
        if (currentCategory !== 'all') items = items.filter(function(m) { return matchesCategory(m, currentCategory); });
        if (currentTranslator !== 'all') items = items.filter(function(m) { return (m.translator_name || '').trim().toLowerCase() === currentTranslator.toLowerCase(); });
        return items;
    }
    ...
    function renderAll() {
        ...
        var latest = getLatestFilms().filter(function(m) {
            return matchesCategory(m, currentCategory) &&
                   (currentTranslator === 'all' || (m.translator_name || '').trim().toLowerCase() === currentTranslator.toLowerCase());
        });
    ...
    function showContent() {
        ...
        // Header, subheader, translator, bottom nav are already visible
        // But we still need to init search, infinite scroll, pull-to-refresh
        initSearch();
        initInfiniteScroll();
        initPullToRefresh();
        loadMyMoviesCount();
    ...
    function init() {
        ...
        // Initialize categories immediately (even before movies load)
        initCategories();
        initCardClick();
        initSecurity();
        initPullToRefresh();

        // Start anonymous session and load movies
