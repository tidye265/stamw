// non.js (Updated: Banner shows always until dismissed, and appears inside header)
...
(function() {
    ...
    //   - Malawi Time = Central Africa Time (CAT) = UTC+2, no DST
    //   - Banner imawoneka nthawi zonse (mpaka user adine X)
    //   - Close X dismisses for current session only
    //   - On reload, banner reappears
    //
    //   KUTI MUSINTHE KUKHALA TIME-BASED (12:00 cutoff):
    //   Uncomment mzere wa "TIME-BASED MODE" pansipa
    // ════════════════════════════════════════════════════════════
    function getMalawiTime() {
        var now = new Date();
        var utcMs = now.getTime();
        var malawiMs = utcMs + (2 * 60 * 60 * 1000); // +2 hours
        var malawiDate = new Date(malawiMs);
        return {
            hour: malawiDate.getUTCHours(),
            minute: malawiDate.getUTCMinutes(),
            date: malawiDate
        };
    }

    function formatMalawiClock() {
        var t = getMalawiTime();
        var h = t.hour;
        var m = t.minute;
        var ampm = h >= 12 ? 'PM' : 'AM';
        ...
        if (h12 === 0) h12 = 12;
        var mm = m < 10 ? '0' + m : m;
        return h12 + ':' + mm + ' ' + ampm + ' CAT';
    }

    function checkMaintenanceBanner() {
        var banner = document.getElementById('maintenanceBanner');
        if (!banner) return;

        var t = getMalawiTime();
        var clockEl = document.getElementById('malawiClock');
        if (clockEl) clockEl.textContent = formatMalawiClock();

        // ════════════════════════════════════════════════════════
        // DEFAULT: Show banner ALWAYS (until dismissed)
        // ════════════════════════════════════════════════════════
        if (!banner.dataset.dismissed) {
            banner.classList.add('visible');
        }

        // ════════════════════════════════════════════════════════
        // TIME-BASED MODE — uncomment below and comment out above
        //   Izi zimaonetsa banner ngati Malawi hour < 12 basi
        // ════════════════════════════════════════════════════════
        /*
        if (t.hour < 12) {
            if (!banner.dataset.dismissed) {
    ...
    function initMaintenanceBanner() {
        ...
        // Close X button (dismiss for current session only)
        var closeBtn = document.getElementById('maintenanceClose');
        if (closeBtn) {
            closeBtn.addEventListener('click', function() {
                banner.classList.remove('visible');
                banner.dataset.dismissed = '1'; // session-only
            });
        }

        // Re-check every 30 seconds (for clock update / auto-hide)
        setInterval(checkMaintenanceBanner, 30000);
    }
