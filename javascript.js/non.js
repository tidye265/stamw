// non.js (Updated with Real Categories & Empty State)
(function() {
    'use strict';
    ...
    function renderGrid(container, items) {
        if (!items || items.length === 0) {
            // Professional empty state with real HTML/CSS
            container.innerHTML = `
                <div class="empty-state">
                    <img src="images/coming-soon.svg" alt="Coming Soon" onerror="this.src='images/logo-akimark.png'; this.style.opacity='0.3';">
                    <h3>More Films Coming</h3>
                    <p>We're adding new content every day. Stay tuned!</p>
                </div>
            `;
            return;
        }
    ...
    function renderCategories() {
        ...
        cats.forEach(function(c) {
            ...
            var displayName = c.charAt(0).toUpperCase() + c.slice(1);
            html += '<button class="category-chip" data-category="' + c + '">' + displayName + '</button>';
        });
    ...
    function getLatestFilms() {
        ...
        return allFilms.slice(0, 6); // Fallback to first 6
    }
    ...
    function renderAll() {
        ...
        // Latest Section: Only show if latest films exist (with current filters)
        var latestSectionTitle = document.querySelector('.section-title');
        if (latest.length > 0) {
            latestSectionTitle.style.display = 'flex';
            renderGrid(latestGrid, latest);
        } else {
            latestSectionTitle.style.display = 'none';
    ...
    async function fetchMovies() {
        try {
            ...
            // Render categories & translators based on real data
            renderCategories();
            renderTranslatorBar();
            ...
            latestGrid.innerHTML = '<div class="empty-state"><img src="images/coming-soon.svg" onerror="this.style.display=\'none\'"><h3>Network Error</h3><p>Please check your connection and try again.</p></div>';
            homeGrid.innerHTML = '<div class="empty-state"><img src="images/coming-soon.svg" onerror="this.style.display=\'none\'"><h3>Network Error</h3><p>Please check your connection and try again.</p></div>';
        }
    }
    ...
    function init() {
        ...
        // Initialize card click immediately
        initCardClick();
        initSecurity();

        // Fetch movies (no need to wait for anonymous session for this)
        fetchMovies();

        // Load my movies count (needs anonymous session)
