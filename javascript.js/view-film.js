(function() {
    ...
    function checkLogin() {
        ...
        .then(function(data) {
            ...
            if (user) {
                setUser(user);
                loadFilm();
                loadTranslatorProfile(); // Load translator profile after auth
            }
        })
        .catch(function() {
            clearSession();
    ...
    function populateFilmData(movie) {
        ...
        // Set PAY NOW button with price
        var price = Number(movie.price || 0);
        payBtnText.textContent = 'PAY NOW MK ' + price.toLocaleString();

        // Store translator name for profile
        if (movie.translator_name) {
            translatorName.textContent = movie.translator_name;
            // Show translator profile section (will fetch avatar/followers via social-api)
