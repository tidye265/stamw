(function() {
    ...
    // LOAD FILM
    function loadFilm() {
        if (!filmId || !isValidUUID(filmId)) { showError('Invalid film ID format. Please use a valid UUID.'); return; }
        ...
        .then(function(response) {
            if (!response.ok) return response.text().then(function(text) { throw new Error('HTTP ' + response.status + ': ' + text); });
        ...
        .catch(function(err) { console.error('Error loading film:', err.message); if (skeletonLoader) skeletonLoader.classList.remove('show'); showError(err.message); });
    }
    ...
    // LOAD TRANSLATOR PROFILE (v1-admin-api)
    function loadTranslatorProfile() {
        if (!filmData || !filmData.translator_name) return;
        // Find admin by name? We need admin_id. For now, use translator_name as query, but backend expects admin_id. We can adapt: use action=get_profile&name=... (but backend we wrote uses admin_id). Let's adjust: we'll use translator_id column? Actually we don't have translator_id in movies, only translator_name. For now, we'll add a fallback: if translator_name matches admin full_name, get admin_id via a separate lookup. To keep simple, we'll modify backend to accept name. Let's update backend to support name.
        // Since backend now expects admin_id, we need to first fetch admin by name. We'll call v1-admin-api with action=find_by_name.
    }
    ...
    function handlePayment() {
        ...
        .then(function(data) {
            ...
            if (data.success) {
                // Show success with real circle check SVG
                ...
                if (data.video_uid) {
                    ...
                    // Don't redirect to home, show message
                    statusMessage.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 6L9 17l-5-5"/></svg> Payment Success! No video yet.';
                }
            } else {
                // Error - don't redirect
                payBtnText.textContent = 'PAY NOW MK ' + (filmData.price || 0).toLocaleString();
