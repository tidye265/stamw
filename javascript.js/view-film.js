...
(function() {
    ...
    function populateFilmData(movie) {
        ...
        else console.error('filmPoster not found');
    ...
    function loadTranslatorProfile() {
        ...
        .then(data => {
            if (data.success && data.admin) {
                ...
                // Use profile image or default icon (SVG data URI)
                if (data.admin.profile_image) {
                    translatorAvatar.src = data.admin.profile_image;
                } else {
                    // Default profile icon (SVG)
                    translatorAvatar.src = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23A7A7A7"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>';
                }
                translatorFollowers.textContent = data.admin.followers_count + ' followers';
                isFollowing = data.admin.is_following;
                updateFollowButton();
            } else {
                // Default icon if no admin data
                translatorAvatar.src = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23A7A7A7"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>';
            }
        })
        .catch(err => { 
