(function() {
    ...
    async function refreshToken() {
        ...
        try {
            const res = await fetch(`${SUPABASE_URL}/functions/v1/viewing-film-api?action=refresh`, {
                method: 'POST',
    ...
    async function fetchWatchInfo() {
        ...
        const res = await fetch(`${SUPABASE_URL}/functions/v1/viewing-film-api?action=get_watch_info`, {
            ...
            headers: {
                ...
                'x-app-proof': token // extra header
            },
        ...
        if (!data.success) {
            if (res.status === 401) {
                ...
                if (newToken) {
                    ...
                    return fetchWatchInfo(); // Retry with new token
                }
                clearSession();
                return null;
            }
