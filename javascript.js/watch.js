(function() {
    ...
    // Test stream URL with a HEAD request to see actual error
    async function validateStreamUrl(streamUrl) {
        try {
            const response = await fetch(streamUrl, { method: 'HEAD' });
            if (!response.ok) {
                const errorText = await response.text();
                console.error('Stream URL error:', response.status, errorText);
                throw new Error(`Stream server returned ${response.status}: ${errorText}`);
            }
            return true;
        } catch (e) {
            console.error('Stream validation failed:', e.message);
            // Try fetching with Range to get more info
            try {
                const rangeResponse = await fetch(streamUrl, { headers: { 'Range': 'bytes=0-0' } });
                if (!rangeResponse.ok) {
                    const errorText = await rangeResponse.text();
                    throw new Error(`Stream server returned ${rangeResponse.status}: ${errorText}`);
                }
            } catch (rangeError) {
    ...
    video.addEventListener('error', (e) => {
        ...
        if (!isExpired) {
            if (video.error?.code === 4) {
                showError('Video file not found or unavailable. Please contact support.');
            } else if (video.error?.code === 3) {
                showError('Video decoding error. The file may be corrupted.');
            } else {
    ...
    async function init() {
        ...
        try {
            ...
            // Validate stream URL before assigning
            // Since we added HEAD support, we can validate
            // But if HEAD fails, it might be because backend doesn't support HEAD.
            // We'll attempt to load anyway and let error handler catch issues.
            video.src = data.stream_url;
