(function() {
    ...
    // Refresh stream URL before expiry
    ...
    video.addEventListener('error', (e) => {
        ...
        showLoader(false);
        if (!isExpired) showError('Video failed to load. Please check your connection and retry.');
    });
            ...
            if (streamUrl.includes('.m3u8')) {
                if (Hls.isSupported()) {
                    ...
                    hls.on(Hls.Events.ERROR, (event, data) => {
                        if (data.fatal) {
                            showError('Video stream error. Please refresh.');
                        }
                    });
                    window.hls = hls;
                } else if (video.canPlayType('application/vnd.apple.mpegurl')) {
