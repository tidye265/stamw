(function() {
    ...
    async function refreshStreamUrl() {
        ...
        try {
            const data = await fetchPlaylistUrl();
            if (!data || !data.stream_url) throw new Error('No stream url');
            setupHls(data.stream_url);
            // After setting up, we wait for the playlist to load and then restore position
            // But HLS might reset; we'll approximate by setting currentTime after loadedmetadata
            // For simplicity, we'll just rely on the new stream being loaded.
    ...
    async function init() {
        ...
        try {
            ...
            // Use HLS.js to load the playlist
            setupHls(data.stream_url);
