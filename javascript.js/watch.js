(function() {
    ...
    const REFRESH_TOKEN_KEY = 'akmark_refresh_token';
    ...
    async function refreshToken() {
        ...
        try {
            const res = await fetchWithTimeout(`${SUPABASE_URL}/functions/v1/viewing-film-api?action=refresh`, {
                method: 'POST',
    ...
    async function fetchWatchInfo() {
        ...
        console.log("🎬 Fetching watch info...");
        const res = await fetchWithTimeout(`${SUPABASE_URL}/functions/v1/viewing-film-api?action=get_watch_info`, {
            method: 'POST',
            headers: {
        ...
        console.log("✅ Watch info received:", data.movie.title);
        return data;
    }
    ...
    function loadResumeTime() {
        ...
        if (!stored) return 0;
        try { return JSON.parse(stored).time || 0; } catch(e) { return 0; }
    }
    ...
    video.addEventListener('error', () => {
        ...
        showLoader(false);
        if (!isExpired) showError('Video failed to load. Please check your connection and try again.');
    });
