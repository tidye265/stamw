(function() {
    ...
    // Validate token format: must have at least one dot (custom token: payload.hash)
    function isTokenValidFormat(token) {
        return token && token.includes('.') && token.split('.').length === 2;
    }

    // Try to refresh token using refresh_token (if exists)
    async function refreshToken() {
        const refreshToken = getRefreshToken();
        if (!refreshToken) return null;

        try {
            const res = await fetch(`${SUPABASE_URL}/functions/v1/security-api?action=refresh`, {
                method: 'POST',
    ...
    async function getValidToken() {
        ...
        if (!isTokenValidFormat(token)) {
            ...
            if (!token || !isTokenValidFormat(token)) {
                clearSession(); // Redirect to non.html
                return null;
            }
        }
        return token;
    ...
    async function fetchWatchInfo() {
        ...
        const res = await fetch(`${SUPABASE_URL}/functions/v1/viewing-film-api?action=get_watch_info`, {
            ...
            headers: {
                ...
                'x-app-proof': token // Extra header for security
            },
