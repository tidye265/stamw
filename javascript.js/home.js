(function() {
    ...
    function showToast(msg, icon) {
        ...
        toast.querySelector('svg').className = icon;
    ...
    function renderTranslatorGroups(container, items) {
        if (!items || items.length === 0) {
            container.innerHTML = '<div class="empty-state"><svg width="48" height="48" viewBox="0 0 24 24" fill="currentColor"><path d="M18 4l2 4h-3l-2-4h-2l2 4h-3l-2-4H8l2 4H7L5 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V4h-2z"/></svg><h4>No movies found</h4><p>Try adjusting your search or filter.</p></div>';
            return;
        }
    ...
    // LOGIN CHECKER – user alibe token kapena invalid, apita ku /register
    function checkLogin() {
        ...
        .catch(function() {
            // Network error – mwina token yachepa kapena server sili kugwira
            // Izi zikufuna kuti tiziyesera kumupititsa ku register kuti asagwire vuto
            localStorage.removeItem('akimark_token');
            window.location.href = '/register';
        });
    }
    ...
    function init() {
        ...
        // After a delay, render actual data
        setTimeout(function() {
            renderAll();
        }, 800);
        ...
        bottomNav.forEach(function(a) {
            a.addEventListener('click', function(e) {
                ...
                if (page === 'search-mobile') { toggleSearchContainer(); return; }
                if (page === 'account') { navigateTo('account'); return; }
                navigateTo(page);
            });
        });
        ...
        playPauseBtn.addEventListener('click', function() {
            ...
            if (icon.getAttribute('data-icon') === 'pause') {
        ...
        searchInput.addEventListener('input', function() {
            renderGrid(homeGrid, filterBySearch(MOVIES, this.value));
        });
        ...
        function toggleSearchContainer() {
            var isVisible = searchContainer.style.display === 'block';
