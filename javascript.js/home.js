(function() {
    ...
    var translatorBar = $('#translatorBar');
    ...
    function movieCardHTML(movie) {
        return '<div class="movie-card" data-id="' + movie.id + '">' +
            ...
            (movie.translator ? '<span class="translator-tag">' + movie.translator + '</span>' : '') +
            '</div>' +
            '<div class="info">' +
            '<h3>' + movie.title + '</h3>' +
            (movie.translator ? '<div class="translator-name">' + movie.translator + '</div>' : '') +
            '</div>' +
            '</div>';
    }
    ...
    function filterByTranslator(items, translator) {
        ...
        return items.filter(function(m) { return m.translator === translator; });
    }
    ...
    function getUniqueTranslators() {
        ...
        allFilms.forEach(function(m) {
            if (m.translator) translators[m.translator] = true;
        });
    ...
    function renderTranslatorBar() {
        ...
        var html = '<button class="translator-chip active" data-translator="all">All</button>';
    ...
    function renderAll() {
        ...
        renderGrid(moviesGrid, filterByCategory(MOVIES, currentCategory).filter(function(m) {
            return currentTranslator === 'all' || m.translator === currentTranslator;
        }));
    ...
    function showMovieDetail(movieId) {
        ...
        var tags = (movie.genre || movie.category) + (movie.translator ? ' • ' + movie.translator : '');
    ...
    function initSecurity() {
        ...
        // Watermark on screenshot attempt (simulate) - actually show watermark on key combos
    ...
    function init() {
        ...
        subheaderChips.forEach(function(chip) {
            chip.addEventListener('click', function() {
                ...
                var filtered = filterByCategory(MOVIES, currentCategory).filter(function(m) {
                    return currentTranslator === 'all' || m.translator === currentTranslator;
                });
                renderGrid(homeGrid, filterByCategory(allFilms, currentCategory).filter(function(m) {
                    return currentTranslator === 'all' || m.translator === currentTranslator;
                }));
                renderGrid(categoriesGrid, filterByCategory(allFilms, currentCategory).filter(function(m) {
                    return currentTranslator === 'all' || m.translator === currentTranslator;
                }));
                ...
                // Also update latest
                var latest = filterByCategory(allFilms, currentCategory).filter(function(m) {
                    return currentTranslator === 'all' || m.translator === currentTranslator;
                }).slice(0, 6);
                renderGrid(latestGrid, latest);
            });
