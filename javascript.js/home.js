(function() {
    ...
    function movieCardHTML(movie) {
        return '<div class="movie-card" data-id="' + movie.id + '">' +
            ...
            (movie.translator_name ? '<span class="translator-tag">' + movie.translator_name + '</span>' : '') +
            '</div>' +
            '<div class="info">' +
            '<h3>' + movie.title + '</h3>' +
            (movie.translator_name ? '<div class="translator-name">' + movie.translator_name + '</div>' : '') +
            '</div>' +
            '</div>';
    }
    ...
    function renderAll() {
        ...
        // Latest films: use latest=true flag, otherwise fallback to first 6
        ...
        renderGrid(moviesGrid, filterByCategory(MOVIES, currentCategory));
        renderGrid(seriesGrid, filterByCategory(SERIES, currentCategory));
    ...
    function init() {
        ...
        subheaderChips.forEach(function(chip) {
            chip.addEventListener('click', function() {
                ...
                var filtered = filterByCategory(allFilms, currentCategory);
                renderGrid(homeGrid, filtered);
                renderGrid(categoriesGrid, filtered);
                renderGrid(moviesGrid, filterByCategory(MOVIES, currentCategory));
                renderGrid(seriesGrid, filterByCategory(SERIES, currentCategory));
