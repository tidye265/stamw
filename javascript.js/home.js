(function() {
    ...
    var cardLoadingOverlay = $('#cardLoadingOverlay');
    ...
    // Updated: include series_id in data attribute and URL
    function movieCardHTML(movie) {
        var seriesParam = movie.series_id ? '&series_id=' + movie.series_id : '';
        return '<div class="movie-card" data-id="' + movie.id + '" data-series-id="' + (movie.series_id || '') + '">' +
    ...
    function initSearch() {
        ...
        searchResults.addEventListener('click', function(e) {
            ...
            if (id) {
                cardLoadingOverlay.classList.add('active');
                var movie = getMovieById(id);
                var seriesParam = movie && movie.series_id ? '&series_id=' + movie.series_id : '';
                setTimeout(function() {
                    window.location.href = 'view-film.html?id=' + id + seriesParam;
                }, 300);
            }
            searchSubheader.classList.remove('open');
            searchResults.classList.remove('visible');
    ...
    detailWatchBtn.addEventListener('click', function() {
        ...
        var seriesParam = currentMovie.series_id ? '&series_id=' + currentMovie.series_id : '';
        window.location.href = 'view-film.html?id=' + currentMovie.id + seriesParam;
    });
    ...
    // Update notification count (placeholder - replace with actual fetch)
    function updateNotificationCount() {
        var notifCount = document.getElementById('notifCount');
        if (notifCount) {
            var count = 0; // Replace with real count from backend
    ...
    function init() {
        ...
        document.querySelectorAll('.movie-grid').forEach(function(grid) {
            grid.addEventListener('click', function(e) {
                ...
                var seriesId = card.dataset.series_id || '';
                if (id) {
                    // Show loading overlay
                    cardLoadingOverlay.classList.add('active');
                    setTimeout(function() {
                        window.location.href = 'view-film.html?id=' + id + (seriesId ? '&series_id=' + seriesId : '');
                    }, 300);
                }
            });
        });
