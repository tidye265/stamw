(function() {
    ...
    function movieCardHTML(movie) {
        return '<div class="movie-card" data-id="' + movie.id + '" data-series-id="' + (movie.series_id || '') + '">' +
            ...
            '<h3>' + movie.title + '</h3>' +
            '</div>' +
            '<div class="card-loading-overlay"><div class="card-loading-spinner"></div></div>' +
            '</div>';
    }
    ...
    // ===== CARD CLICK WITH LOADING OVERLAY =====
    function handleCardClick(card) {
        // Show loading overlay
        var overlay = card.querySelector('.card-loading-overlay');
        if (overlay) overlay.classList.add('active');

        var id = card.dataset.id;
        var seriesId = card.dataset.seriesId;

        // Redirect using series_id if available
        if (seriesId) {
            window.location.href = 'view-film.html?series_id=' + seriesId;
        } else {
