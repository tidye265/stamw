(function() {
    ...
    function checkLogin() {
        ...
        .then(function(data) {
            ...
            if (data.user) localStorage.setItem('akmark_user', JSON.stringify(data.user));
        })
        .catch(function() { clearSession(); window.location.href = '/register'; });
    }
    ...
    // ===== MACHOTSEDWA: TRANSLATORS HARDCODED =====
    // (Zonse zomwe zinali za getTranslatorForMovie, TRANSLATORS, zatsalidwa)
    ...
    // ===== RENDER SIMPLE GRID (Chotsa translator grouping) =====
    function renderGrid(container, items) {
        ...
        container.innerHTML = '<div class="movie-grid">' + html + '</div>';
    }
    ...
    async function fetchMovies() {
        try {
            ...
            // Standalone movies vs series episodes
            ...
            if (MOVIES.length === 0) MOVIES = allMovies; // fallback
    ...
    function navigateTo(page) {
        ...
        var sections = { home: $('#pageHome'), movies: $('#pageMovies'), series: $('#pageSeries'), categories: $('#pageCategories') };
        for (var key in sections) if (sections.hasOwnProperty(key)) sections[key].classList.remove('active');
    ...
    function initSearch() {
        searchNavBtn.addEventListener('click', function(e) { e.preventDefault(); toggleSearchSubheader(); });
        searchInput.addEventListener('input', function() {
            var query = this.value.toLowerCase().trim();
            if (!query) { searchResults.classList.remove('visible'); return; }
            var filtered = MOVIES.filter(function(m) { return m.title.toLowerCase().includes(query) || m.genre.toLowerCase().includes(query); });
    ...
    function init() {
        ...
        desktopNav.forEach(function(a) {
            a.addEventListener('click', function(e) { e.preventDefault(); navigateTo(this.dataset.page); });
        });

        bottomNav.forEach(function(a) {
            a.addEventListener('click', function(e) {
                if (this.id === 'searchNavBtn') { e.preventDefault(); toggleSearchSubheader(); return; }
                // default navigation (if href set)
            });
        });

        logoHome.addEventListener('click', function() { window.location.href = 'home.html'; });
        ...
        detailWatchBtn.addEventListener('click', function() { if (!currentMovie) return; openPlayer(); });
        expiredWatchBtn.addEventListener('click', function() { if (!currentMovie) return; showToast('Access expired. Purchase needed.'); });

        closePlayer.addEventListener('click', closePlayer);
        playPauseBtn.addEventListener('click', function() {
            var icon = this.querySelector('i');
            if (icon.classList.contains('bi-pause-fill')) { icon.className = 'bi-play-fill'; clearInterval(playbackInterval); playbackInterval = null; }
            else { icon.className = 'bi-pause-fill'; simulatePlayback(); }
        });
        volumeBtn.addEventListener('click', function() { var icon = this.querySelector('i'); icon.className = icon.classList.contains('bi-volume-up-fill') ? 'bi-volume-mute-fill' : 'bi-volume-up-fill'; });
        fullscreenBtn.addEventListener('click', function() { if (!document.fullscreenElement) { if (document.documentElement.requestFullscreen) document.documentElement.requestFullscreen(); } else { if (document.exitFullscreen) document.exitFullscreen(); } });
        progressTrack.addEventListener('click', function(e) { var rect = this.getBoundingClientRect(); var pct = (e.clientX - rect.left) / rect.width; playbackProgress = Math.min(100, Math.max(0, pct * 100)); progressFill.style.width = playbackProgress + '%'; });

        movieSearchInput.addEventListener('input', function() { renderGrid(moviesGrid, filterBySearch(MOVIES, this.value)); });
        clearMovieSearch.addEventListener('click', function() { movieSearchInput.value = ''; renderGrid(moviesGrid, MOVIES); });
    ...
    function renderSkeleton() {
        var skeletonHTML = '';
        for (var i = 0; i < 6; i++) skeletonHTML += '<div class="skeleton-card"></div>';
