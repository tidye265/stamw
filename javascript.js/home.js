(function() {
    ...
    function movieCardHTML(movie) {
        var translatorName = movie.translators ? movie.translators.name : 'Unknown';
        return '<div class="movie-card" data-id="' + movie.id + '">' +
            ...
            '<div class="info">' +
            '<h3>' + movie.title + '</h3>' +
            '<p style="font-size:0.7rem;color:var(--text-muted);margin-top:2px;">' + translatorName + '</p>' +
            '</div>' +
            '</div>';
    }
    ...
    function renderTranslatorGroups(container, items) {
        ...
        items.forEach(function(movie) {
            var translatorName = movie.translators ? movie.translators.name : 'Unknown';
            if (!groups[translatorName]) {
                groups[translatorName] = { name: translatorName, movies: [] };
            }
            groups[translatorName].movies.push(movie);
        });
