...
(function() {
    ...
    function getUniqueTranslators() {
        ...
        allFilms.forEach(function(m) {
            if (m.translator_name) {
                ...
                var key = name.toLowerCase(); // case-insensitive
                if (!seen[key]) {
                    seen[key] = true;
    ...
    // ===== CATEGORY MATCHING (handle multiple categories comma-separated) =====
    function matchesCategory(movie, category) {
        if (category === 'all') return true;
        if (!movie.category) return false;
        // Check if category is an array? We'll treat as string.
    ...
    function renderAll() {
        ...
        var latest = getLatestFilms().filter(function(m) {
            return matchesCategory(m, currentCategory) && (currentTranslator === 'all' || (m.translator_name || '').trim().toLowerCase() === currentTranslator.toLowerCase());
        });
    ...
    async function fetchMovies() {
        try {
            ...
            renderTranslatorBar();
            renderAll();
            hideSkeletons(); // remove skeleton after render
            initSearch();
            initInfiniteScroll();
            loadMyMoviesBadge();
            loadNotifications();
    ...
    // ===== PULL TO REFRESH (Robust, works on APK) =====
    ...
    // ===== SEARCH (More reliable with debounce) =====
    function initSearch() {
