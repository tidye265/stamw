...
(function() {
    ...
    async function confirmPayment() {
        ...
        try {
            const session = await ensureSession();
            // NOTE: Simulate payment success here. In real integration, call PayChangu.
            // For demo, we just call watch action with price_paid.
            const res = await fetch(`${SUPABASE_URL}/functions/v1/non-viewing-film-api?action=watch`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${session.access_token}` },
                body: JSON.stringify({ movie_id: movieId, price_paid: 0 }) // Replace with actual price if needed
            });
            const data = await res.json();
            if (!data.success) throw new Error(data.error || 'Payment failed');

            // Close modal and start player
            modalOverlay.classList.remove('active');
            startPlayer(data.video_url, data.expire_at);
