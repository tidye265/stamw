...
(function() {
    ...
    // ===== NETWORK DETECTION (From deposit.js) =====
    ...
    async function loadMovie() {
        try {
            ...
            // Check if user already has active access (non_views)
    ...
    // ===== CONFIRM PAYMENT (PayChangu Integration) =====
    async function confirmPayment() {
        ...
        // Get current movie price from DOM (number)
        ...
        try {
            ...
            // Step 2: Poll verify until success/fail
            const verifyInterval = setInterval(async () => {
                const verifyRes = await fetch(`${SUPABASE_URL}/functions/v1/non-deposit-api`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                    body: JSON.stringify({
                        action: 'verify',
                ...
                if (verifyData.success && verifyData.status === 'success') {
                    ...
                    // Payment successful – fetch video URL
                    const watchRes = await fetch(`${SUPABASE_URL}/functions/v1/non-viewing-film-api?action=watch`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                        body: JSON.stringify({ movie_id: movieId })
                    });
                    ...
                    if (watchData.success) {
                        modalOverlay.classList.remove('active');
                        startPlayer(watchData.video_url, watchData.expire_at);
                    } else {
                        alert('Payment done but unable to load video. Try again.');
                    }
                    confirmBtn.disabled = false;
                    confirmBtn.textContent = 'PAY NOW';
                } else if (verifyData.success && verifyData.status === 'failed') {
                    clearInterval(verifyInterval);
